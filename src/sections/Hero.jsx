...
const LiveClock = ({ className = "" }) => {
  // Null until mounted so the clock never causes an SSR hydration mismatch
  // (server bakes a static date; the client ticks from now).
...
const CTA_STYLES = {
  primary:
    "group items-center  bg-foreground px-6 py-3 text-sm font-bold font-hud uppercase tracking-widest text-background transition-transform hover:-translate-y-0.5",
  outline:
    "items-center uppercase border border-foreground/40 px-6 py-3 font-hud text-sm  tracking-widest text-foreground transition-colors hover:border-foreground hover:bg-foreground hover:text-background",
...
const renderCta = (b, i, go) => {
  ...
  // Internal CTAs route with the deck's power swap (not a default jump).
  const onClick = (e) => {
    e.preventDefault();
    go(b.href);
  };
  if (b.kind === "primary") {
    return (
      <a key={i} href={b.href} onClick={onClick} className={cls}>
              ...
              if (it.value) {
                return (
                  <span key={i} className={hide}>
                    <b className="text-foreground font-medium">{it.value}</b>
                    {it.text}
                  </span>
                );
              }
